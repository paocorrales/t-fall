...
# 
file_list_cmip <- readr::read_rds(here::here("cmip_file_list_historical.rds")) |>
  _[nominal_resolution != "500 km"]
...
purrr::map(11:nrow(file_list_cmip), function(m) {
...
# furrr::future_map(1:nrow(file_list_cmip), function(m) {
  ...
  file_list <- data.table(path = files, purrr::map_df(files, extract_range)) |> 
    _[, merge := fifelse(inidate >= 1850 | enddate >= 2014, TRUE, FALSE)] |> 
    _[merge == TRUE]
  
  message(paste0(".........", m, ".........."))
  
  outfile <- paste0("~/t-fall/data/cmip/historical/", "deltat_day_", model, "_", experiment, "_", member, "_", grid_label, "_20150101-21001231.nc")
  ...
  # shifted_f <- cdo_seldate(regridded, startdate = "2015-01-02", enddate = "2100-12-31") |>
  #   cdo_execute(options = "-L")
  # shifted_b <- cdo_seldate(regridded, startdate = "2015-01-01", enddate = "2100-12-30") |>
  #   cdo_execute(options = "-L")
  # 
  # cdo_sub(shifted_f, shifted_b)
})
