...
wind <- ReadNetCDF(paste0("/scratch/w40/pc2687/cf/u_era5_oper_pl_", format(id_date, "%Y-%m-%d"), ".nc"),
                                      vars = "u",
                                      subset = list(time = as.list(dates),
                                                    level = 850)) |> 
    ReadNetCDF(paste0("/scratch/w40/pc2687/cf/v_era5_oper_pl_", format(id_date, "%Y-%m-%d"), ".nc"),
               vars = "v",
               subset = list(time = as.list(dates),
                             level = 850))[i = _, on = c("time", "latitude", "longitude", "level")] |> 
  _[, level := NULL]
...
field |> 
  ...
  geom_contour2(aes(z = msl/100, label = after_stat(level)), color = "grey10", 
               ...
               label_size = 2.5,
               label_color = "grey10") +
  # geom_contour2(aes(z = thickness, label = after_stat(level)), color = "red4", 
  #               # breaks = seq(900, 1020, 2),
  #               linewidth = 0.3,
  #               label_size = 2.5,
  #               label_color = "red4",
  #               linetype = 2) +
  geom_arrow(aes(angle = atan2(dlat(v), dlon(u, latitude))*180/pi,
                 mag = Mag(v, u)), skip = 8, pivot = 0.5,
  ...
  labs(x = NULL, y = NULL, fill =  NULL, mag =  NULL,
       ...
       subtitle = "MSLP (black), Wind 850 hPa (m/s), Precip (mm/h)") +
  theme_minimal(base_size = 10) +
  theme(legend.key = element_rect(fill = "white", color = "white"),
        plot.title.position = "plot")

ggsave(paste0("event_", format(date, "%Y-%m-%d"), "_sfc.png"), width = 15, height = 9, bg = "white")
